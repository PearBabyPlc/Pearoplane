...
int main() {
	...
	Camera camera = {0};
	camera.position = (Vector3){30.0f, 10.0f, 30.0f};
	...
	while (!WindowShouldClose()) {
		frameTime = GetFrameTime() * 50.0f * (float)isPaused;
		...
		if (camTarg == 0) {
			camera.target = posSun;
			if (IsKeyPressed(KEY_RIGHT_BRACKET) || IsKeyPressed(KEY_LEFT_BRACKET)) camera.position = (Vector3){30.0f, 10.0f, 30.0f};
		}
		if (camTarg == 1) {
			camera.target = posEarth;
			if (IsKeyPressed(KEY_RIGHT_BRACKET) || IsKeyPressed(KEY_LEFT_BRACKET)) camera.position = (Vector3){posEarth.x + 5.0f, posEarth.y + 2.0f, posEarth.z + 5.0f};
		}
		if (camTarg == 2) {
			camera.target = posMoon;
			if (IsKeyPressed(KEY_RIGHT_BRACKET) || IsKeyPressed(KEY_LEFT_BRACKET)) camera.position = (Vector3){posMoon.x + 2.0f, posMoon.y + 1.0f, posMoon.z + 2.0f};
		}
